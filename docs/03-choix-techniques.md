...
| ------------------ | ------------------------------------------ | ------------------------------------------------------------------- |
| Runtime LLM local  | **Ollama**                                 | Simple, API standard, modèles quantisés — **seul actif en V1**      |
| Modèle             | **léger : Gemma / Qwen / Phi**             | Choisi au banc d'essai (16 Go, sans GPU)                            |
| UI de chat brute   | **OpenWebUI** (optionnel)                  | Interface de secours / debug modèles                                |
| Fournisseurs Cloud | **OpenAI, Claude, Gemini**                 | ⛔ **hors cœur V1** — adaptateurs optionnels, désactivés par défaut |
| Abstraction        | **Provider pluggable** (port `AIProvider`) | Changer de moteur sans changer le code                              |
...
> **Décision validée ([ADR-0006](adr/ADR-0006-kai-ia-locale.md))** : KAI est
> **100 % local, gratuit, hors-ligne** en V1 (Ollama + modèle léger), **sans
> dépendance GPU**. Le moteur est un **fournisseur interchangeable** : brancher
> un fournisseur externe plus tard n'impacte pas le cœur. Attentes calibrées :
> KAI est un **assistant système** (façon Jarvis), pas un chatbot généraliste.
